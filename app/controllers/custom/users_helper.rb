...
module UsersHelper
  def proposal_limit_exceeded?(user)
    user.proposals.where(retired_at: nil).count >=
      Setting["extended_option.proposals.max_active_proposals_per_user"].to_i
  end
  ...
  def options_for_gender_select
    [
      [t("cli.gender.male"), "male"],
      [t("cli.gender.female"), "female"]
    ]
  end
end
