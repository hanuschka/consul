require_dependency Rails.root.join("app", "controllers", "verification","sms_controller").to_s
class Verification::SmsController < ApplicationController
  skip_before_action :verify_resident!
  ...
  def update
    ...
    if @sms.verified?
      ...
      current_user.update!(confirmed_phone: current_user.unconfirmed_phone, document_number: current_user.id.to_s, document_type: '1')
