class DocumentsQuery
  ALLOWED_SORTS = {
    "created_desc" => "documents.created_at DESC",
    "created_asc"  => "documents.created_at ASC",
    "updated_desc" => "documents.updated_at DESC",
    "updated_asc"  => "documents.updated_at ASC",
  ...
  def call
    scope = Document.joins(attachment_attachment: :blob)
  ...
  private
    ...
    def filter_search(scope)
      ...
      scope.where(
        "LOWER(documents.title) LIKE :q OR LOWER(active_storage_blobs.filename) LIKE :q",
        q: like
      )
    end
    ...
    def filter_created(scope)
      scope = apply_date_lower_bound(scope, :created_from, "documents.created_at")
      apply_date_upper_bound(scope, :created_to, "documents.created_at")
    end

    def filter_updated(scope)
      scope = apply_date_lower_bound(scope, :updated_from, "documents.updated_at")
      apply_date_upper_bound(scope, :updated_to, "documents.updated_at")
    end

    def filter_documentable_type(scope)
      value = read_param(:documentable_type)
