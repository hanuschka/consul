class Whatsapp::AiAssistant::OptOutIntentService < ApplicationService
  ...
  # happened before this service existed — and can never leave a typed "STOP"
  # unhonoured or revoke an opt-out already written.
  ...
  def call
    ...
    Rails.logger.error("[Whatsapp] opt-out intent failed: #{e.class} - #{e.message}")
  ...
  private
    ...
    # silences a channel the citizen wanted — so the instruction asks for
    ...
    # "abbrechen" and "doch nicht" end the draft and nothing else, and reading
    # one of them as leaving the channel would unsubscribe someone who only
    # changed their mind about one idea.
    def instructions
      <<~TEXT
        ...
        nachrichten mehr", "nehmt mich von der liste", "ich möchte nichts mehr von euch hören",
        "hör auf mir zu schreiben", "please stop messaging me".
    ...
    def output_schema
      {
        ...
        properties: {
          opt_out: {
            type: "boolean",
            description: "True only when the message unmistakably asks to stop receiving " \
                         "messages from this channel altogether."
          }
        },
        required: %w[opt_out],
