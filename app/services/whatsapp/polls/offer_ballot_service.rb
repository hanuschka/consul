class Whatsapp::Polls::OfferBallotService < ApplicationService
  ...
  # can carry to the end. Answers whether it did: a false is the caller's signal to
  # fall back to the link, which is what every poll this cannot ask gets — and the
  # link now opens the ballot itself rather than the projekt page that lists it.
  #
  ...
  private
    ...
    def begin_ballot(poll)
      ...
      # Its answer is this one's. A ballot with nothing left to ask is a citizen who
      # has already voted on every question of it, and the closing line says so; a
      # ballot that could not be carried on has cleared its own markers and left the
      # link as the right thing to send.
