          ...
          under **API → API clients**. Creating a client there also
          ...
          Rotating issues a new token and invalidates the current one
          immediately, so every integration still using the old token starts
          receiving `401 Unauthorized`. Update your integrations right after a
          rotation.

          **In the admin area (recommended):** go to **API → API clients**, open
          the client, and press **Regenerate token** on its detail page. Confirm
          the dialog; the new token is then displayed in the *Auth token* field,
          where it can be revealed and copied.

          **Via the API:** an authenticated client can rotate its own token with
          `POST /api/auth/rotate_token`; the response returns the new token.
