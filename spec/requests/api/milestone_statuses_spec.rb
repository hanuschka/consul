...
RSpec.describe 'Milestone Statuses API', type: :request, openapi_spec: 'v1/swagger.yaml' do
  ...
  path '/api/milestones/{milestone_id}/milestone_statuses' do
    parameter name: :milestone_id, in: :path, type: :integer, description: 'Milestone ID'

    post 'Create a milestone status for a milestone' do
      tags 'Milestone Statuses'
      consumes 'application/json'
      produces 'application/json'
      security [bearer_auth: []]
      ...
      response '201', 'milestone status created' do
        ...
        let(:phase) { projekt.projekt_phases.create!(type: 'ProjektPhase::QuestionPhase', active: true) }
        let(:milestone) do
